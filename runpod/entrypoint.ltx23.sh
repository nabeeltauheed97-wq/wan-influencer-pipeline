#!/usr/bin/env bash
# RunPod ComfyUI + LTX-2.3 container entrypoint (parallel to entrypoint.sh).
#
# Responsibilities:
#   1. Resolve env-var driven config (workflow, source, node IDs, S3, etc.)
#   2. Download LTX-2.3 FP8 dev checkpoint, IC-LoRA Union Control, Gemma
#      text encoder, and DWPose ONNX weights via `hf download` (skippable)
...
# 1) Env / defaults
# ---------------------------------------------------------------------------
WORKFLOW="${WORKFLOW:-workflows/ltx23_pose.json}"
...
if [[ "$SKIP_MODEL_DOWNLOAD" != "1" ]]; then
    echo "[entrypoint] Downloading LTX-2.3 model weights"

    # LTX-2.3 FP8 dev checkpoint (~22 B params, ~22 GB FP8). Lives in
    # models/checkpoints/ per Lightricks docs and the example workflow's
    # CheckpointLoaderSimple node. Single .safetensors bundles diffusion
    # weights + VAE; no separate VAELoader needed.
    download_model \
    ...
    # IC-LoRA Union Control adapter — drives pose / depth / canny / motion-
    # track conditioning. The pose workflow consumes this via
    ...
    # Gemma text encoder. ComfyUI-LTXVideo's LTXAVTextEncoderLoader looks
    # for the encoder under models/text_encoders/<folder>/. We pull the
    # full HF snapshot into that folder.
    if [[ -f "$COMFY_ROOT/models/text_encoders/gemma-3-12b-it-qat-q4_0-unquantized/config.json" ]]; then
        echo "  [skip] Gemma text encoder already present"
    else
        mkdir -p "$COMFY_ROOT/models/text_encoders/gemma-3-12b-it-qat-q4_0-unquantized"
        echo "  [pull] google/gemma-3-12b-it-qat-q4_0-unquantized -> $COMFY_ROOT/models/text_encoders/gemma-3-12b-it-qat-q4_0-unquantized"
        hf download google/gemma-3-12b-it-qat-q4_0-unquantized \
            --local-dir "$COMFY_ROOT/models/text_encoders/gemma-3-12b-it-qat-q4_0-unquantized"
    fi

    # DWPose: two files. Existence check on dw-ll_ucoco_384.onnx covers
    # both. Same wiring as the Wan track — DWPreprocessor consumes these.
